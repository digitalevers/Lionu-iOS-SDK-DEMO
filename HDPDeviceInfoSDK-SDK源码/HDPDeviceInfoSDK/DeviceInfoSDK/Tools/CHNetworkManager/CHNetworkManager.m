...
+ (instancetype)shareInstance{
    static CHNetworkManager *_manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _manager = [[CHNetworkManager alloc] init];
        [_manager configure];
    });
    return _manager;
}
...
/**基础配置*/
- (void)configure{
    self.hostURL = [HDPDeviceInfoSDK shareInstance].configModel.HostUrl;
}
...
- (NSDictionary *)getCommonParams{
    ...
    NSString * appID = [HDPDeviceInfoSDK shareInstance].configModel.APPID;
    if (appID.length > 0) {
...
- (void)requestHTTPMethod:(NSString *)httpMenthod relativePath:(NSString *)relativePath params:(NSDictionary *)params successBlock:(CHResponseSuccessBlock)successBlock failBlock:(CHResponseFailBlock)failBlock{
    ...
    if ([HDPDeviceInfoSDK shareInstance].configModel.isLog) {
